...
public sealed unsafe class Window
{
    ...
    public Window(string title, int x, int y, int width, int height, WindowFlags flags, bool threadedProcessing)
    {
    ...
    public Window(IntPtr windowHandle, bool threadedProcessing)
    {
        ...
        {
            _window = CreateWindowFrom(windowHandle);
            WindowId = GetWindowID(_window);
            EventProcessor.RegisterWindow(this);
            PostWindowCreated(0);
        }
    ...
    public WindowState WindowState
    {
        get
        {
            WindowFlags flags = GetWindowFlags(_window);
        ...
        set
        {
            switch (value)
            {
                case WindowState.Normal:
                    SetWindowFullscreen(_window, FullScreenMode.Windowed);
                    break;
                case WindowState.FullScreen:
                    SetWindowFullscreen(_window, FullScreenMode.Fullscreen);
                    break;
                ...
                case WindowState.BorderlessFullScreen:
                    SetWindowFullscreen(_window, FullScreenMode.FullScreenDesktop);
                    break;
    ...
    public bool Visible
    {
        get => (GetWindowFlags(_window) & WindowFlags.Shown) != 0;
        set
        {
            if (value)
            {
    ...
    public float Opacity
    {
        get
        {
            ...
            if (GetWindowOpacity(_window, &opacity) == 0)
            {
                return opacity;
            }
            return float.NaN;
        ...
        set
        {
            SetWindowOpacity(_window, value);
        }
    }

    public bool Focused => (GetWindowFlags(_window) & WindowFlags.InputFocus) != 0;

    public bool Resizable
    {
        get => (GetWindowFlags(_window) & WindowFlags.Resizable) != 0;
        set => SetWindowResizable(_window, value ? 1u : 0u);
    }

    public bool BorderVisible
    {
        get => (GetWindowFlags(_window) & WindowFlags.Borderless) == 0;
        set => SetWindowBordered(_window, value ? 1u : 0u);
    }
    ...
    {
        EventProcessor.RemoveWindow(this);
        Closing?.Invoke();
    ...
    private void ProcessEvents(SdlEventHandler eventHandler)
    {
        ...
        lock (EventProcessor.Lock)
        {
            EventProcessor.PumpEvents();
    ...
    private void CheckNewWindowTitle()
    {
        if (WindowState != WindowState.Minimized && _newWindowTitleReceived)
        {
            ...
            Sdl2Native.SetWindowTitle(_window, _cachedWindowTitle);
        }
    }
    ...
    private void HandleMouseMotionEvent(SDL_MouseMotionEvent mouseMotionEvent)
    {
        ...
        var delta = new Vector2(mouseMotionEvent.x - _currentMouseX, mouseMotionEvent.y - _currentMouseY);
    ...
    private void HandleKeyboardEvent(SDL_KeyboardEvent keyboardEvent)
    {
        ...
        var keyEvent = new KeyEvent(MapKey(keyboardEvent.keysym), keyboardEvent.state == 1, MapModifierKeys(keyboardEvent.keysym.mod), (char)keyboardEvent.keysym.sym);
    ...
    private void RefreshCachedSize()
    {
        int w;
        int h;
        Sdl2Native.GetWindowSize(_window, &w, &h);
    ...
    {
        Sdl2Native.SetWindowPosition(_window, x, y);
        _cachedPosition.Value = new Point(x, y);
    }
    ...
    {
        Sdl2Native.SetWindowSize(_window, width, height);
    ...
    private class WindowParams
    {
        ...
        public WindowPtr Create()
        {
            if (WindowHandle != IntPtr.Zero)
            {
                return CreateWindowFrom(WindowHandle);
            }
            else
            {
